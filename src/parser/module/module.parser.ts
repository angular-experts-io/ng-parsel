...
function getDecoratorPropertyValue(ast: ts.SourceFile, identifier: string): string[] {
    return [...tsquery(ast, `Decorator > CallExpression > ObjectLiteralExpression > PropertyAssignment:has(Identifier[escapedText="${identifier}"]) > ArrayLiteralExpression > Identifier`)]
        .map((identifier: any) => identifier.getText());
}

function getDecoratorPropertyObject(ast: ts.SourceFile, identifier: string): string[] {
    return [...tsquery(ast, `Decorator > CallExpression > ObjectLiteralExpression > PropertyAssignment:has(Identifier[escapedText="${identifier}"]) > ArrayLiteralExpression > ObjectLiteralExpression`)]
